...
### Correção: FAQ só responde automaticamente quando a pergunta é IDÊNTICA a uma já cadastrada (nunca por semelhança aproximada)

Silvan reportou (com prints de uma conversa real) que "Posso comer mandioca?" foi respondido corretamente (sobre aipim), mas a pergunta seguinte, "POsso comer batata?", recebeu **exatamente a mesma resposta** (sobre aipim) - o que não fazia sentido, já que batata e aipim são alimentos diferentes.

**Investigação**: o motor de alimento/medicamento (`buscar_resposta_alimento`/`buscar_resposta_medicamento`, em `app/faq_engine.py`) não tinha nenhum problema - o culpado era `buscar_resposta()` (a busca na base de FAQ, `FaqItem`), que fazia correspondência **por semelhança aproximada** (contagem de palavras-chave em comum + um limiar de confiança, `LIMIAR_CONFIANCA = 0.45`). O Silvan tinha aprovado antes uma pergunta parecida sobre "aipim" pela tela do médico (`/equipe/perguntas`), o que criou automaticamente um `FaqItem` (ver `app.routes_medico.perguntas_responder`). Como "posso comer mandioca?" e "posso comer batata?" compartilham várias palavras com "posso comer aipim?" (a pergunta que gerou aquele FaqItem), ambas bateram por semelhança aproximada acima do limiar e foram respondidas na hora com o texto do aipim - **sem passar pela aprovação do médico de novo**, e sem nenhuma relação real de conteúdo.

Esse é o MESMO mecanismo de um bug pré-existente já documentado (mas não corrigido) em `test_smoke.py`: uma FAQ aprendida da IA sobre "gatorade de uva" sendo reaproveitada indevidamente para uma pergunta sobre "gatorade de limão" - havia até um guard tentando evitar isso (`item.criado_por == "Assistente (IA)"`), mas ele nunca disparava de verdade, porque `app.routes_medico.perguntas_responder` sempre grava o nome de quem aprovou (o médico/secretária), nunca esse valor literal.

**Pedido do Silvan para a correção**: "só deve voltar a resposta automática ao paciente (sem passar por IA), se a pergunta for idêntica à que já existir no FAQ."

**Implementado em `app/faq_engine.py`**:
- `buscar_resposta()` reescrita: agora só casa quando a pergunta do paciente, depois de normalizada (`normalizar()` - minúsculas, sem acento, sem pontuação), é **igual** à de algum `FaqItem` já cadastrado no mesmo escopo (Grupo, ou dono pessoal em conta solo). Nunca mais por contagem de palavras-chave/limiar de confiança.
- Removidos: a função `similaridade()` (comparação de strings via `difflib.SequenceMatcher`) e a constante `LIMIAR_CONFIANCA = 0.45`, que só eram usadas ali. **Atenção**: `from difflib import SequenceMatcher` continua importado no topo do arquivo, porque `_palavras_correspondem()` (usada por `buscar_resposta_alimento`/`buscar_resposta_medicamento`, para tolerar erros de digitação como "amendoin" → "amendoim") também usa `SequenceMatcher` - só a comparação de PERGUNTAS INTEIRAS (`buscar_resposta`) deixou de usar semelhança aproximada; a tolerância a erro de digitação PALAVRA A PALAVRA no motor de alimento/medicamento continua igual (fora do escopo deste pedido).
- Esta mudança corrige os dois bugs de uma vez (o do Silvan, sobre "mandioca"/"batata"/"aipim", e o pré-existente sobre "gatorade de uva"/"limão"), já que os dois tinham a mesma causa raiz.
- A busca por FAQs de alimento/medicamento cadastrados manualmente (não só as geradas pela IA) também passa a exigir igualdade exata - antes, só as FAQs geradas pela própria IA tinham essa exigência; agora é uniforme para toda a base de FAQ.

**Testes** (`test_smoke.py`):
- Corrigido um caso que já estava incompatível com esta mudança: a pergunta enviada pelo paciente ("Posso comer batata antes do exame?") não batia EXATAMENTE com o texto da FAQ seedada em `seed.py` ("Posso comer batata antes da colonoscopia?") - ajustada a pergunta do teste para o texto idêntico ao cadastrado (comportamento esperado agora: só responde igualzinho).
- Simplificado o bloco que documentava o bug pré-existente "gatorade de uva/limão" (não precisa mais do `if/else` condicional - agora sempre resolve para o caminho corrigido, já que a causa raiz foi eliminada).
- **Lacuna encontrada e corrigida nesta mesma rodada** (não diretamente ligada a este pedido, mas teria deixado a suíte de testes quebrada): a correção anterior desta sessão ("resposta automática de alimento/medicamento também passa a exigir aprovação do médico", ver seção acima) tinha alterado o comportamento de `app/routes_paciente.py:chat()`, mas várias asserções em `test_smoke.py` que testavam alimento/medicamento (água de coco, leite e derivados, amendoim, AAS, Xarelto, laranja/banana via categoria "Frutas") ainda esperavam resposta instantânea direta ao paciente - foram todas reescritas para confirmar que a resposta calculada fica em `PerguntaPendente` (`status="aguardando_aprovacao"`, com `resposta_sugerida_ia` já preenchida com o texto certo) em vez de ir direto ao paciente.

- **Pendência**: mesma de sempre - rodar `python test_smoke.py` (ou `test_smoke_final.py`) e as suítes de WhatsApp antes de subir pra produção; não executado aqui (`device_bash` continua indisponível nesta máquina). Depois do deploy, vale reproduzir o cenário original do Silvan (perguntar sobre "mandioca", depois sobre "batata", em conversas separadas) para confirmar visualmente que cada uma recebe a resposta certa (ou fica pendente de aprovação, se ainda não houver FAQ idêntica cadastrada) em vez de reaproveitar uma resposta de outro alimento.

### Correção: aviso duplicado ao paciente (mesma mensagem processada duas vezes pela reentrega do webhook da Meta)

Silvan reportou (com print de uma conversa real) o mesmo aviso "Sua pergunta ainda está sendo respondida pela equipe..." chegando **duplicado** ao paciente, em bubbles separados. Investigando: não é um bug de lógica de conversa - é a Cloud API da Meta reentregando o **mesmo webhook** mais de uma vez (comportamento documentado dela, mais comum quando o processamento demora um pouco para devolver 200) - sem nenhuma proteção contra isso, `processar_mensagem` rodava do zero para a mesma mensagem já processada, e como a `PerguntaPendente` da primeira rodada já existia, a segunda rodada só repetia o aviso de "aguardando resposta" como uma mensagem nova.

**Implementado**:
- Novo modelo `WhatsappMensagemProcessada` em `app/models.py` (tabela nova, `whatsapp_mensagens_processadas`, com `mensagem_id` único) - registra o campo `"id"` que a Meta atribui a cada mensagem (único por mensagem, segundo a própria documentação da Meta). Tabela nova não precisa de `ALTER TABLE` - `db.create_all()` (chamado em `create_app`, roda em todo start do app) cria sozinha, mesmo padrão já usado antes para `conversas_whatsapp`/`push_subscriptions`.
- `app/routes_whatsapp.py`: `_extrair_mensagens_de_texto` agora também devolve o `mensagem_id` de cada mensagem (além de telefone/texto); nova função `_mensagem_ja_processada(mensagem_id)` consulta (e já registra, no mesmo passo) esse id antes de processar - se já existir, a mensagem é ignorada (só um log, sem reprocessar). Um `IntegrityError` no INSERT (duas requisições quase simultâneas para o mesmo id) também é tratado como duplicado, sem quebrar o webhook.
- `webhook()` (POST) agora pula mensagens já vistas antes de chamar `processar_mensagem`/`enviar_mensagem_whatsapp` - sempre continua respondendo 200 pra Meta, mesmo pra reentregas (nunca queremos que ela reentregue de novo por isso).

**Testes**: `test_whatsapp_webhook_assinatura.py` ganhou dois casos novos - reenviar o MESMO payload (mesmo `id` de mensagem) com assinatura válida de novo confirma que `processar_mensagem` NÃO é chamada de novo (mock) e que não duplica o registro em `WhatsappMensagemProcessada`; e uma mensagem com `id` novo continua sendo processada normalmente (a dedupe é só por id, não trava conversas novas).

- **Pendência**: rodar a suíte completa (não executado aqui, `device_bash` continua indisponível) antes de subir pra produção. Depois do deploy, o próprio Silvan pode confirmar visualmente que o aviso duplicado não volta a aparecer (é mais fácil de reproduzir em cenários onde o processamento demora um pouco, ex.: quando a IA é chamada).
