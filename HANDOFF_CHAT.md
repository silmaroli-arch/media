...
### Correção: "Method Not Allowed" ao salvar um modelo de preparo importado de PDF pelo celular

Silvan reportou (com print de tela do celular) o erro "Method Not Allowed" ao tentar salvar um modelo de preparo depois de importar um PDF pela tela reduzida do celular (menu "Exames & preparo" no celular → botão "Importar de um PDF", ver `medico.preparo_modelos_aviso_mobile`).

**Causa**: o popup de importação (`app/templates/medico/_importar_preparo_pdf.html`) extrai o PDF e, ao concluir, substitui a página inteira via `document.write(html)` pela tela de revisão completa (`medico/preparo_modelo_form.html`) - isso troca o CONTEÚDO da página, mas não muda a URL na barra de endereço, que continua sendo `/preparo-modelos/aviso-mobile` (a tela do celular, que só aceitava GET). O `<form id="form-preparo">` dessa tela de revisão não tem `action` definido DE PROPÓSITO - funciona tanto para criar (`/preparo-modelos/novo`) quanto para editar (`/preparo-modelos/<id>/editar`), cada um submetendo pra si mesmo, sem o formulário precisar saber qual dos dois é. Só que isso pressupõe que a URL atual do navegador é uma das duas - e no celular, depois da troca de página via `document.write`, a URL continha era `/preparo-modelos/aviso-mobile`, que não tinha handler de POST nenhum → 405.

**Implementado em `app/routes_medico.py`** (`preparo_modelos_aviso_mobile`):
- Rota passou a aceitar `methods=["GET", "POST"]` (antes só GET).
- No POST, delega direto para `preparo_modelos_novo()` (a mesma função usada por `/preparo-modelos/novo`) - a importação pelo celular só cria modelo NOVO, nunca edita um existente, então não precisa de nenhuma lógica própria, só reaproveitar a que já existe.
- Docstring da rota estendida explicando a causa raiz (útil se um bug parecido aparecer em outra tela que reaproveite o mesmo popup no futuro).

**Testes**: novo `test_preparo_importar_pdf_mobile.py` - confirma que o GET na tela continua funcionando, e que um POST na MESMA URL (reproduzindo o que o navegador manda depois da troca de página via `document.write`, sem simular o popup/JS em si) não dá mais 405 e cria o modelo de preparo normalmente.

- **Pendência**: mesma de sempre - `device_bash` indisponível nesta rodada também, então nada disso foi executado (só validado por leitura + `ast.parse`). Rodar `python test_preparo_importar_pdf_mobile.py` (e a suíte completa) antes de subir pra produção, e confirmar na prática pelo celular: importar um PDF pela tela reduzida e salvar o modelo revisado.
