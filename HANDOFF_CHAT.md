...
## Bug corrigido: recusa disfarçada da IA no imperativo não era detectada (2026-09-24)

Caso real relatado pelo Silvan (print de tela do WhatsApp): paciente perguntou "Posso comer maconha?" e a Clara respondeu direto (sem passar pelo médico):

> "Não há nenhuma informação sobre maconha nas orientações de preparo deste exame. Para esclarecer essa dúvida específica, entre em contato com a secretaria da clínica ou com o médico responsável."

Pedido do Silvan: "Perguntas em que a IA não acha uma resposta, deve sempre passar pelo médico" - e esse já É o comportamento pretendido do sistema (ver `MARCADOR_NAO_SEI` e a "rede de segurança contra recusa disfarçada", `_eh_recusa_generica_disfarcada`, em `app/ia_preparo.py` - quando a IA não sabe a resposta, `resultado_ia["final"]` fica `None` e `app.whatsapp_conversa._responder_pergunta` SEMPRE cria uma `PerguntaPendente` pro médico nesse caso, **independente** da configuração de aprovação automática do Grupo/médico - só respostas de verdade, com `final` preenchido, é que podem ir direto pro paciente quando a aprovação automática está desativada).

**Causa raiz encontrada**: o texto da IA batia com o primeiro padrão do regex de recusa disfarçada (`_PADROES_SEM_INFORMACAO`, "não há nenhuma informação..."), mas NÃO batia com o segundo padrão (`_PADROES_ENCAMINHA_PARA_CLINICA`), que só reconhecia a forma infinitiva `"entrar em contato"` - a IA respondeu no imperativo, `"entre em contato"`, que o regex antigo não cobria. Como as duas partes do padrão são exigidas juntas (deliberadamente conservador, ver docstring do módulo), a recusa disfarçada passou batido como se fosse uma resposta de verdade, e como esse médico/grupo tem a aprovação automática ativada (`aprovacao_perguntas_paciente = False`), foi direto pro paciente sem ninguém revisar.

**Correção** (`app/ia_preparo.py`, `_PADROES_ENCAMINHA_PARA_CLINICA`): trocado `entrar em contato` (forma fixa) por `entr\w* em contato` (cobre entrar/entre/entrei/entrando etc.) e adicionado `procur\w*` (ex.: "procure a secretaria") como mais uma forma comum de encaminhamento que ainda não estava coberta.

**Testes**: `test_ia_recusa_disfarcada.py` ganhou 3 casos novos - o caso real da maconha (texto exato do print), uma variação com "entrando em contato", e uma variação com "procure a secretaria" - todos confirmando que agora são reconhecidos como recusa disfarçada (checagem por texto puro, sem precisar chamar IA de verdade nem banco).
...
## Tela pública de preparo (link no WhatsApp) + PDF baixável (2026-09-24)

Pedido do Silvan, a partir de um print de tela real (paciente perguntou "Posso chupar bala Halls?" e recebeu datas calculadas): "Na mensagem ali de baixo vamos colocar um link que leva para uma tela no sistema que mostra as informações do preparo do exame. Essa tela ainda não existe, vais criar." Esclarecido por 3 perguntas (respostas do Silvan): (1) o link é por **token único por agendamento** (sem exigir login - o paciente do WhatsApp nunca fez login no site); (2) a tela é **um documento único** (não as 6 abas do cadastro do preparo) com cabeçalho (médico, data do exame, nome do exame), **todos os itens com prazo numa única linha do tempo cronológica** ("tipo: dia 22 suspender monjauro etc"), e um botão para **baixar em PDF**; (3) o link aparece **só na mensagem que convida a perguntar** ("Pode escrever sua pergunta sobre o preparo deste exame"), não em toda mensagem do fluxo.

**Modelo de segurança (combinado com o Silvan)**: sem login - quem tiver o link exato consegue ver, igual ao link de resultado de exame por WhatsApp que já existia antes neste sistema (`app.push_notificacoes`). O token é opaco (`secrets.token_urlsafe(32)`, mesmo padrão de `Usuario.codigo_mestre`/`GrupoConvite` já usados no projeto), sem nenhuma informação do agendamento embutida nele.

**`app/models.py`** (`Agendamento`):
- `token_preparo_publico` (String(43), unique, nullable) - vazio até a primeira vez que o link for montado (gerado sob demanda, não em todo agendamento).
- `obter_token_preparo_publico()` - gera o token na primeira chamada e reaproveita nas seguintes.

**`app/preparo_publico.py`** (novo módulo - fonte ÚNICA da montagem desses dados, reaproveitada pela tela HTML e pelo PDF, pra nunca divergirem uma da outra):
- `montar_link_preparo(agendamento)` - gera/reaproveita o token e **comita imediatamente** se ele acabou de ser criado agora (garante que sobrevive mesmo que o resto da transação da requisição não seja comitado por outro motivo depois). Constrói a URL manualmente com `APP_URL_PUBLICA` (env var, default `https://media-dev.onrender.com`) em vez de `url_for(_external=True)` - **de propósito**: este projeto não tem `ProxyFix`/`SERVER_NAME` configurado, e `url_for(_external=True)` pode gerar `http://` mesmo em produção atrás do proxy do Render (mesmo problema já documentado e contornado em `app.push_notificacoes._link_perguntas`).
- `montar_documento_preparo(agendamento)` - monta a `linha_do_tempo` (cortes, medicamentos suspensos, alimentos proibidos-com-prazo, exames anteriores proibidos-com-prazo, informações gerais-com-prazo, mais o próprio exame no final), **ordenada cronologicamente**, e separa em `sem_data` (dict de listas) tudo que não tem prazo calculável: medicamentos mantidos, alimentos permitidos, alimentos/exames proibidos sem prazo específico, informações soltas. Sem preparo cadastrado, devolve linha do tempo vazia (tela/PDF mostram a mensagem de sempre, "não é necessário preparo prévio").
- `gerar_pdf_preparo(documento)` - PDF via reportlab/Platypus (já dependência do projeto, mesmo padrão de estilo de `app.relatorios_utils.exportar_pdf`: `SimpleDocTemplate` + `Table` com cabeçalho azul `#0d6efd`). `_texto_seguro()` escapa `&`/`<`/`>` antes de qualquer texto livre entrar num `Paragraph` (que interpreta um mini-XML) - sem isso, um preparo com "chá & água" no texto de instruções quebraria a geração do PDF inteiro.

**`app/routes_paciente.py`**: duas rotas novas, **sem** `@login_required`/`@paciente_required` (de propósito - é a tela pública):
- `GET /paciente/preparo/<token>` (`preparo_publico`) - renderiza `paciente/preparo_publico.html`.
- `GET /paciente/preparo/<token>/pdf` (`preparo_publico_pdf`) - devolve o PDF (`Content-Disposition: attachment`).
- As duas fazem `Agendamento.query.filter_by(token_preparo_publico=token).first_or_404()` - token errado ou vazio dá 404 normal, nunca expõe se o agendamento existe.

**`app/templates/paciente/preparo_publico.html`** (novo, standalone - não estende `base.html`, é uma página HTML completa própria): cabeçalho com nome do exame, data/hora e médico; botão "Baixar PDF"; linha do tempo visual (marcadores coloridos, azul para os itens normais e verde para o item final "Exame: ..."); seções abaixo para os itens sem prazo; CSS com `@media print` (some o botão de PDF ao imprimir).

**`app/whatsapp_conversa.py`**: `_texto_pedir_pergunta()` agora inclui `montar_link_preparo(agendamento)` logo antes de "Pode escrever sua pergunta sobre o preparo deste exame." - único ponto onde o link aparece, por pedido explícito do Silvan (não entra em nenhuma outra mensagem do fluxo, ex.: convite de "trocar de exame" ou mensagens de erro).

**Migração (`migrar_banco.py`)**: `ALTER TABLE agendamentos ADD COLUMN IF NOT EXISTS token_preparo_publico VARCHAR(43);` e `CREATE UNIQUE INDEX IF NOT EXISTS uq_agendamentos_token_preparo_publico ON agendamentos (token_preparo_publico);` (unicidade em bases que só ganharam a coluna via `ALTER` - mesmo padrão já usado para `Usuario.codigo_mestre`, mais acima neste arquivo).

**Testes**: `test_preparo_publico.py` (novo) - usa o agendamento de colonoscopia do João (seed.py, com cortes/medicamentos suspensos/mantidos/alimentos já cadastrados), sem mockar nada (não depende de IA): confirma que `montar_link_preparo` gera o token uma vez e reaproveita nas chamadas seguintes (e que o token persiste de verdade no banco); que `montar_documento_preparo` junta os itens certos na linha do tempo, na ordem certa, terminando com o próprio exame, e separa corretamente o que não tem prazo (medicamento mantido, alimento permitido) em `sem_data`; que `gerar_pdf_preparo` devolve `mimetype="application/pdf"` e não quebra com texto livre contendo `&`/`<`/`>`; e que um agendamento sem preparo nenhum (simulado desassociando o modelo em memória, sem comitar) não derruba nem a montagem do documento nem a geração do PDF.

- **Pendência (mesmo padrão de sempre - validado só por `ast.parse` e `jinja2.Environment().parse()` no template, sem `flask_sqlalchemy` disponível neste ambiente de desenvolvimento pra rodar a suíte de ponta a ponta de verdade)**: depois do próximo deploy, testar pelo WhatsApp real: (a) confirmar que a mensagem "Exame em foco: ..." chega com o link; (b) abrir o link num navegador comum (sem estar logado) e confirmar que a tela mostra a linha do tempo certa pro exame/agendamento em questão; (c) clicar em "Baixar PDF" e confirmar que o PDF abre normalmente, com o mesmo conteúdo da tela; (d) confirmar que um token inválido/inventado dá 404, sem vazar nenhuma informação. Vale também confirmar visualmente que a linha do tempo fica legível num celular (tela estreita) - o CSS foi escrito responsivo, mas sem poder testar num navegador de verdade neste ambiente.
...
### Gerar cobrança do ano para todos de uma vez (2026-09-25)

Pedido do Silvan, depois de perguntar onde ficava o link "Pagar agora" (ver seção acima, mesmo dia): em vez do dono gerar a cobrança Mercado Pago usuário por usuário/mês por mês (fluxo que já existia - `usuario_licenca_pagamento_cobrar`), ele quer gerar de uma vez a cobrança dos meses que faltam no ano pra todo mundo. Decidido via pergunta ao Silvan: (1) período = só os meses restantes deste ano civil (do mês seguinte ao atual até dezembro, não o ano inteiro nem 12 meses corridos); (2) mês que já tem cobrança gerada é pulado, nunca duplicado; (3) mês já pago (manualmente ou não) nunca gera cobrança nova.

**`app/models.py`**: `garantir_meses_licenca(usuario, fim=None)` ganhou o parâmetro opcional `fim` (antes só ia até o mês atual) - é o que permite adiantar a criação dos meses futuros (out/nov/dez) antes de gerar a cobrança deles.

**`app/routes_dono.py`**: nova rota `dono.licencas_gerar_cobrancas_ano` (POST) - só afeta médico com `ciclo_licenca == "mensal"` (quem está no ciclo anual usa o próprio botão de cobrança anual, que já cobre o ano inteiro num pagamento único; gerar mensal pra ele duplicaria a cobrança). Pra cada médico mensal: garante que os meses até dezembro existem, então gera a cobrança (`criar_preferencia_pagamento`) de cada mês que ainda não tem `mp_init_point` e ainda não está pago. Se o Mercado Pago não estiver configurado (`MERCADOPAGO_ACCESS_TOKEN` ausente - é o caso do `media-dev` hoje), para tudo na primeira tentativa e avisa, sem deixar nada pela metade. Ao final, mostra um resumo: quantas foram geradas, quantas já tinham cobrança (puladas), quantas não tinham valor mensal definido (puladas) e quais falharam (se alguma).

**`app/templates/dono/usuarios.html`**: botão "Gerar cobranças do ano para todos" no topo da lista de usuários, com `confirm()` explicando o que vai acontecer, e uma nota abaixo explicando os critérios.

**Testes**: `test_licencas_gerar_cobrancas_ano.py` (novo, mesmo padrão de mock de `requests.post` já usado em `test_licenca_pagamento_valor_e_gateway.py` - sem chamada de rede de verdade) - cobre os 4 cenários: médico mensal sem nada gerado (deve gerar todos os meses restantes), médico em ciclo anual (não deve ser tocado), médico mensal sem valor definido (meses são criados mas nenhum ganha link), e médico com um mês já pago + outro já com cobrança gerada (nenhum dos dois é alterado - o mês com cobrança mantém o link ORIGINAL). Também cobre o caso de borda de rodar em dezembro (nenhum mês restante no ano - a rota avisa e não cria nada).

- **Pendência (mesmo padrão de sempre - validado só por `ast.parse`/`jinja2.Environment().parse()`, sem `flask`/`flask_sqlalchemy` instalados neste ambiente de desenvolvimento pra rodar a suíte de ponta a ponta de verdade)**: como o Mercado Pago não está configurado no `media-dev` (sem `MERCADOPAGO_ACCESS_TOKEN` no `.env`), não foi possível testar clicando de verdade neste ambiente - só depois que as credenciais forem cadastradas (produção, ou credenciais de teste do Mercado Pago no dev) dá pra confirmar o fluxo ponta a ponta: clicar no botão, ver o resumo com a contagem certa, e confirmar que o link "Pagar agora" aparece na tela "Minha licença" de cada médico afetado.

## Pix nativo com QR code dentro do sistema (nova nesta sessão)

Pedido do Silvan (2026-09-25), depois de perguntar como disponibilizar Pix pra pagamento: "Quero o qrcode aparecendo dentro do sistema". Decidido via pergunta ao Silvan: (1) Pix é uma opção ADICIONAL ao link de Checkout Pro já existente, não substitui; (2) aparece na tela "Minha licença" do próprio médico (autoatendimento, sem depender do dono); (3) vale tanto pro fluxo individual (um mês por vez) quanto pro fluxo em massa (gerar cobranças do ano pra todos).

Achado-chave de arquitetura (o que reduziu bastante o tamanho da mudança): o webhook em `app/routes_pagamentos_webhook.py` já é agnóstico ao método de pagamento - ele só olha o prefixo da `external_reference` ("licenca_pagamento:" ou "licenca_anual:") pra saber o que fazer, e chama `consultar_pagamento(data_id)` pra confirmar direto na API. Ou seja: bastou o Pix usar o MESMO formato de `external_reference` que o link já usa, e o webhook confirma os dois sem precisar de UMA LINHA de mudança nele.

**`app/models.py`**: `LicencaPagamento` ganhou 4 colunas novas - `pix_qr_code` (Text, código copia-e-cola), `pix_qr_code_base64` (Text, imagem do QR pronta pra `<img src="data:image/png;base64,...">`), `pix_payment_id` (String(80)) e `pix_expira_em` (DateTime, ~30min a partir da geração - prazo padrão de expiração do Pix no Mercado Pago).

**`migrar_banco.py`**: 4 `ALTER TABLE licenca_pagamentos ADD COLUMN IF NOT EXISTS ...` correspondentes, dentro do mesmo bloco SQL de sempre.

**`app/mercadopago_integration.py`**: nova função `criar_cobranca_pix(pagamento)` - usa a Payments API (`POST /v1/payments`, `payment_method_id: "pix"`), NÃO a Preferences API do Checkout Pro (`criar_preferencia_pagamento` continua existindo do jeito que estava, intocada). Usa o mesmo `external_reference` no formato `"licenca_pagamento:<id>"` (por isso o webhook não precisa mudar) e manda um `X-Idempotency-Key` novo (`uuid4`) a cada chamada, exigido pela API pra evitar cobrança duplicada em caso de retry de rede - cada clique em "Gerar Pix" é intencionalmente uma cobrança nova, com prazo de expiração próprio. Levanta `MercadoPagoNaoConfigurado`/`ValueError` nas mesmas condições de `criar_preferencia_pagamento`.

**`app/routes_medico.py`**: nova rota `medico.minha_licenca_gerar_pix` (POST, `/minha-licenca/pagamentos/<id>/pix`) - autoatendimento: o próprio médico gera o Pix pra um mês seu (confere que o pagamento é dele, recusa se já estiver pago), chama `criar_cobranca_pix` e mostra mensagem de sucesso/erro (Mercado Pago não configurado / sem valor definido / falha genérica), igual ao padrão das outras rotas de cobrança.

**`app/routes_dono.py`**: `dono.licencas_gerar_cobrancas_ano` (a geração em massa da sessão anterior) foi estendida - além de gerar o link de Checkout Pro pra cada mês que ainda não tem, agora TAMBÉM gera o Pix pra cada mês que ainda não tem `pix_qr_code`, como uma segunda passada independente (uma pode ter sucesso mesmo se a outra falhar, e vice-versa - por exemplo um mês que já tinha link de uma geração anterior mas ainda não tinha Pix agora recebe só o Pix). Resumo final ganhou linhas próprias pra Pix ("N Pix gerados", "N já tinham Pix", "N Pix falharam").

**`app/templates/medico/minha_licenca.html`**: novo macro Jinja `pix_bloco(p)` (evita duplicar HTML), usado nos dois lugares onde "Pagar agora" já aparecia - o alerta de pagamento anual pendente e cada linha do calendário mensal. Mostra o QR code (`<img>` com o base64) + campo com o código copia-e-cola e botão "Copiar" (`navigator.clipboard.writeText`, só client-side) enquanto `pix_expira_em` não passou; depois de expirado, mostra "Pix anterior expirou" e o botão "Gerar Pix" de novo (sem regeneração automática - decisão de manter simples, o médico clica de novo quando precisar). A view `medico.minha_licenca` passou a mandar `agora=datetime.utcnow()` pro template, usado só pra essa comparação de expiração.

**Testes**: `test_pix_licenca.py` (novo) - cobre: gerar Pix sem credenciais (mensagem clara, nada salvo); gerar Pix com credenciais falsas (QR code/código copia-e-cola salvos e aparecendo na tela, `X-Idempotency-Key` enviado, URL correta da Payments API); o MESMO webhook de sempre confirmando um pagamento que veio via Pix (sem nenhuma mudança no webhook); mês já pago não pode gerar Pix; e a geração em massa do ano também gerando Pix pra cada mês, junto com o link.

- **Pendência (mesmo padrão de sempre - validado só por `ast.parse`/`jinja2.Environment().parse()`)**: sem `MERCADOPAGO_ACCESS_TOKEN` configurado no `media-dev`, não dá pra testar um Pix de verdade neste ambiente - só depois das credenciais (de teste ou produção) dá pra confirmar ponta a ponta: gerar o QR de verdade, escanear com o app do banco, e ver o webhook confirmar o pagamento automaticamente. Também não foi implementada regeneração automática de Pix expirado (o médico precisa clicar em "Gerar Pix" de novo) - se isso incomodar no uso real, pode virar um botão "Atualizar" com auto-refresh da tela, mas não foi pedido ainda.
