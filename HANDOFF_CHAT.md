...
**Pedido relacionado, ainda pendente de decisão do Silvan**: no mesmo pedido, o Silvan também pediu "se pelo menos duas palavras não existirem na frase, dê erro" - ou seja, marcar como sem sentido quando 2+ palavras da mensagem não existem como palavras reais em português. Investiguei viabilidade e não há nenhum dicionário/verificador ortográfico de português disponível neste ambiente de nuvem nem na máquina do Silvan (`hunspell-pt-br`/`aspell`/bibliotecas como `pyenchant` - tentei instalar via `pip` e via `apt-get` nos dois lugares, ambos bloqueados pela mesma política de rede, erro 403). Sem um dicionário de verdade, a alternativa seria um heurística (ex.: palavra sem nenhuma vogal, ou com sequência muito longa de consoantes, "provavelmente não é uma palavra real") - mas isso tem um risco real de falso positivo em abreviações comuns de paciente já testadas e aceitas neste projeto (ex.: "vc", "pq", "tb" não têm vogal) - se duas dessas aparecerem juntas numa pergunta de verdade (ex.: "pq vc disse que não posso comer?"), a heurística marcaria como sem sentido por engano, bloqueando uma pergunta real. Como isso é uma decisão de risco (não uma escolha técnica óbvia), não implementei essa parte ainda - preciso que o Silvan decida entre: (a) não implementar esse item específico (o julgamento por IA implementado na seção anterior já cobre boa parte do mesmo problema - "palavras reais em ordem sem sentido" -, e só depende de haver IA configurada); (b) implementar a heurística de "palavra sem vogal/sequência longa de consoante" mesmo com esse risco de falso positivo; ou (c) outra abordagem que ele prefira.
