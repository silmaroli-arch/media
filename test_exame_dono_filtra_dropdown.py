...
"Exame" do formulário "Adicionar" listava TODOS os exames da empresa,
...
from app.models import Usuario, Clinica, Exame, ClinicaMembro
...
with app.app_context():
    ...
    # Um segundo médico, também vinculado à mesma filial.
    ...
    db.session.flush()
    db.session.add(ClinicaMembro(clinica_id=centro_id, usuario_id=medico_bianca.id, ativo=True))
    db.session.commit()
    ...
    exame_eduardo = Exame(
        clinica_id=centro_id, medico_id=medico_eduardo_id, nome="Exame do Eduardo",
        descricao="", duracao_minutos=30, medico_confirmado=True, criado_por_id=medico_eduardo_id,
    )
    exame_bianca = Exame(
        clinica_id=centro_id, medico_id=medico_bianca_id, nome="Exame da Bianca",
        descricao="", duracao_minutos=30, medico_confirmado=True, criado_por_id=medico_bianca_id,
    )
    exame_sem_dono = Exame(
        clinica_id=centro_id, medico_id=medico_eduardo_id, nome="Exame Sem Dono Registrado",
